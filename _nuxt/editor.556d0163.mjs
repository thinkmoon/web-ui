import { A as gh, T as _T, C as PT, a as wT, l as ra, _ as K6 } from '../server.mjs';
...
const I = [{ name: "thumb", value: "" }, { name: "desc", value: "" }], W = defineComponent({ data() {
  ...
  this.$route.query.cid ? gh.getDetail({ cid: this.$route.query.cid }).then((i) => {
  ...
  gh[i](this.article).then(() => {
    this.$message.success("\u4FDD\u5B58\u6210\u529F"), location.href = "/admin/post-list";
  }).catch((e) => {
    this.$message.error("\u4FDD\u5B58\u5931\u8D25"), console.error(e);
  });
}, handleUploadImage(i, e, a) {
  console.log(arguments), wT.getUploadToken().then((m) => {
    const V = At().format("YYYY-MM-DD/HH-mm-ss");
    oN.upload(a[0], V, m).subscribe(null, null, (A) => {
      e({ url: `https://blog.cdn.thinkmoon.cn/${A.key}` });
    });
  });
} } }), q = defineComponent({ ...W, __name: "editor", __ssrInlineRender: true, setup(i) {
  return (e, a, m, V) => {
    const k = ra.ElInput, A = resolveComponent("v-md-editor"), E = K6, h = ra.ElSelect, p = ra.ElOption;
...
export { q as default };
//# sourceMappingURL=editor.556d0163.mjs.map
